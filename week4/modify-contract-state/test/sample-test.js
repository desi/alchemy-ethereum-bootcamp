...
describe("TestModifyVariable", function () {
  ...
  it("should change xString to The new string", async function () {
    const ModifyVariable = await ethers.getContractFactory("ModifyVariable");

    const contract = await ModifyVariable.deploy(10);

    await contract.deployed();
    await contract.modifyToNewString();

    const newXString = await contract.xString();
    assert.equal(newXString, "The new string");
  });
});